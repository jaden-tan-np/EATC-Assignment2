...
def preprocess_fraud_data(df, label_encoders=None, scaler=None, expected_features=None):
    ...
    if "trans_date_trans_time" in df_clean.columns and "dob" in df_clean.columns:
        df_clean['trans_date'] = pd.to_datetime(df_clean['trans_date_trans_time'], format='mixed', dayfirst=True, errors='coerce')
        df_clean['dob'] = pd.to_datetime(df_clean['dob'], format='mixed', dayfirst=True, errors='coerce')
        df_clean['customer_age'] = (df_clean['trans_date'] - df_clean['dob']).dt.days // 365
        
        # Haversine Distance
        if all(col in df_clean.columns for col in ['lat', 'long', 'merch_lat', 'merch_long']):
    ...
    if "merchant" in df_clean.columns:
        df_clean["merchant"] = df_clean["merchant"].apply(lambda x: str(x).replace("fraud_", ""))
    
    # 3. Drop unused text/unique string identifiers to prevent data leakage
    drop_cols = ["trans_num", "first", "last", "street", "city", "state", "zip", "dob", "cc_num", "trans_date_trans_time", "trans_date"]
    ...
    if label_encoders is not None:
        for col in categorical_cols:
            if col in df_clean.columns:
                ...
                df_clean[col] = df_clean[col].apply(lambda x: x if x in le.classes_ else "Unknown")
                # Handle potential "Unknown" class not present during fitting
                if "Unknown" not in le.classes_:
                    # Fallback safely to first class if Unknown isn't encoded
                    df_clean[col] = df_clean[col].apply(lambda x: x if x in le.classes_ else le.classes_[0])
...
if uploaded_file is not None:
    ...
    if st.button("Scan File for Fraud", type="primary"):
        with st.spinner("Processing data, engineering features, and scanning transactions..."):
...
if "scan_results" in st.session_state and "feature_cols" in st.session_state:
    ...
    if len(df_fraud) == 0:
        ...
        st.dataframe(df_fraud.drop(columns=["Fraud_Prediction"]), use_container_width=True)
